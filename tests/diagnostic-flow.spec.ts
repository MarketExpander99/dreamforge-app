...
test.describe('Diagnostic Assessment Flow', () => {
  ...
  test('Mobile responsiveness test', async ({ page }) => {
    ...
    const nextButtonsMobile = page.locator('button:has-text("Next"), button:has-text("Continue")');
    for (let i = 0; i < 3; i++) {
      await nextButtonsMobile.first().click();
      await page.waitForTimeout(1000);
    }
    ...
    // Take mobile screenshot of start
    console.log('📸 Taking mobile screenshot of diagnostic start');
    await page.screenshot({ path: 'test-results/diagnostic-start-mobile.png', fullPage: true });

    // Answer first few questions on mobile
    const mobileOptions = page.locator('button[class*="option"], button[class*="choice"]');
    if (await mobileOptions.count() > 0) {
      await mobileOptions.first().click();
    }

    const mobileNext = page.locator('button:has-text("Next")');
    ...
    // Take mobile screenshot of in-progress
    console.log('📸 Taking mobile screenshot of diagnostic in-progress');
    await page.screenshot({ path: 'test-results/diagnostic-in-progress-mobile.png', fullPage: true });

    // Complete quiz and submit
    // (Simplified - answer remaining questions)
    for (let q = 1; q < 5; q++) {
      const options = page.locator('button[class*="option"], button:has-text("A"), button:has-text("B")');
      if (await options.count() > 0) {
        await options.first().click();
      }

      const next = page.locator('button:has-text("Next"), button:has-text("Finish")');
    ...
    const mobileSubmit = page.locator('button:has-text("Submit Assessment")');
    ...
    // Take mobile screenshot of results
    console.log('📸 Taking mobile screenshot of diagnostic results');
    await page.screenshot({ path: 'test-results/diagnostic-results-mobile.png', fullPage: true });

    // Verify mobile layout
    const mobileLayoutValid = await page.locator('body').evaluate(() => {
      const body = document.body;
      const width = body.offsetWidth;
      const isMobile = width <= 768;
      return isMobile && !body.classList.contains('overflow-hidden');
    });

    expect(mobileLayoutValid).toBeTruthy();
