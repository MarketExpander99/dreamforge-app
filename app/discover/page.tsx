  ...
  const exploreTopic = async (topic: string) => {
    ...
    try {
      ...
      const newNode: Node = {
        ...
        description: data.description || 'Deep interconnected knowledge awaits exploration.',
        function: data.function || 'Serves as a fundamental building block in its domain.',
        components: data.components || ['Component 1', 'Component 2', 'Component 3'],
      ...
      console.error(e);
      alert('Failed to connect to Grok. Please ensure XAI_API_KEY is set in .env.local');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      {expandedNodes.has(node.id) && node.components.map((comp, i) => (
        <div key={i} className="pl-6 py-1">
          <button 
            ...
            className="text-sm text-purple-300 hover:text-purple-400 flex items-center gap-1"
          >
            → {comp}
          </button>
        </div>
    ...
    <div className="min-h-screen bg-zinc-950 text-white pb-20 lg:pb-6">
      <div className="max-w-7xl mx-auto p-4 lg:p-6">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
          <div>
            <h1 className="text-4xl lg:text-5xl font-bold bg-gradient-to-r from-blue-400 via-purple-500 to-violet-400 bg-clip-text text-transparent">
              Discover
            </h1>
            <p className="text-zinc-400">Infinite E8 knowledge lattice • Powered by Grok</p>
          </div>
          <Badge className="text-xs">Quantum Entanglement Mode</Badge>
        </div>

        <div className="flex gap-3 mb-8">
          <Input
            placeholder="Type anything... PC, Accountant, Grade 12, Cheese Burger..."
            value={searchQuery}
              ...
              <CardContent className="p-0 h-[calc(100%-4.5rem)] relative">
                {isMobile || !centerNode ? (
                  <div className="p-6 overflow-auto h-full bg-zinc-950">
                    {centerNode ? (
                      <TreeView node={centerNode} />
                    ) : (
                      <div className="flex flex-col items-center justify-center h-full text-center text-zinc-500">
                        <p className="text-lg">What would you like to explore today?</p>
                        <p className="text-sm mt-3">Search anything. Drill infinitely deep.</p>
                      </div>
                    )}
                  </div>
                ) : (
            ...
            {centerNode ? (
              ...
              <Card className="bg-zinc-900 border-zinc-700 h-full flex items-center justify-center text-center p-10">
                <div className="max-w-xs">
                  ...
                  <p className="text-sm text-zinc-500 mt-3">Start exploring the E8 lattice.</p>
                </div>
              </Card>
            )}
          </div>
